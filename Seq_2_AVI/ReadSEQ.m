function [failPath] = ReadSEQ(varargin,frames,failPath)
% -------------------------------------------------------------------------
...
%if varargin is one instead of filepath, choose file
if varargin == 1
    [ name,seqPath ] = uigetfile({'*.seq'},'Pick a video seq. file...');
    
   fileName = [seqPath name];
   
   [seqPath,name,ext] = fileparts(fileName);
   
   fileName = [seqPath filesep name ext];
   
%else use inputed file path    
elseif varargin == 2
    
    % get main directory
    [mainPath] = uigetdir('Select Main Directory');
    
    % get the locations of all seq files
    cd(mainPath);
%     output = dir('**/*.seq');
%     
%     %loop through and list the path/file names of all .seq files
%     for i = 1:numel(output)
%         seqPath{i,1} = ([output(i).folder filesep output(i).name]);
%     end
    load seqPathsToDo
...
%     failPath{1,1} = '.';
    ...
    % run through the list of seqences
   for j = 1:numel(seqPath) %failed on OG 14 - to finish conversions
       
       fTest = numel(failPath);

       
       [failPath] = ReadSEQ(seqPath{j,1},[0 0],failPath);
       
       if numel(failPath) > fTest
           display(['Seq'  num2str(j) ' not converted -----'])
       else
            display(['Seq' num2str(j) ' CONVERTED -----'])
           
       end
      
       save([mainPath filesep 'SEQPaths.mat'], 'failPath', 'seqPath')
       
   end

    
else
    [seqPath,name,ext] = fileparts(varargin);
...
vOut = VideoWriter([VoutPath filesep name 'C'],'Motion JPEG AVI');
...
%         I{i,1} = SEQ;
        ...
        writeVideo(vOut,SEQ);
        display(['done frame: ' num2str(i) ' of ' num2str(readAmount)]);
    end
