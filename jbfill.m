...
if length(upper)==length(lower) && length(lower)==length(xpoints)
    ...
    % display a warning if there are nan's, fill won't work properly
    if ~isempty(find(double(isnan([upper lower xpoints]))==1,1,"first"))
    warning('cannot properly use fill when data contains nan')
    end
